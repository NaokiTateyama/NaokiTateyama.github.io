...
function Profile() {
  return (
    <Box>
      <H2 id={ID.ABOUT} title={'About'} top={true} />
      ...
      {information.jobs.map((job, i) => {
        return <ItemJob key={`${ID.JOB_EXPERIENCE}-${i}`} job={job} />;
      })}

      <H2 id={ID.EDUCATION} title={'Education'} />
      <Divider sx={{ borderBottomWidth: 2 }} />
      {information.education.map((education, i) => {
        return <ItemEdu key={`${ID.EDUCATION}-${i}`} education={education} />;
      })}
      ...
      {information.publications.journal.map((publication, i) => {
        return <ItemPub key={`${ID.JOURNAL}-${i}`} publication={publication} />;
      })}
  ...
  return (
    <>
      <Box mt={2} mb={2} ml={1}>
        <Typography>
          {<b>Languages: </b>}
          {props.programming.language.join(', ')}
        </Typography>
      </Box>
      <Box mt={2} mb={2} ml={1}>
        <Typography>
          {<b>Framework: </b>}
          {props.programming.framework.join(', ')}
        </Typography>
      </Box>
    </>
  );
